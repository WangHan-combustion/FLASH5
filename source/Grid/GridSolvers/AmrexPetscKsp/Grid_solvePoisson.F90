...
!  Select BCs to send to AMReX poisson solver
     do i=1,6
       ...
       case default
          call Driver_abortFlash('[Grid_solvePoisson] BC not implemented for PETSc solver!')
       end select
     end do
...
! solution multifab's ghost cells at physical boundaries have been set to bc values.
       ...
       call multigrid % set_max_fmg_iter(gr_amrexLs_max_fmg_iter)
       call multigrid % set_bottom_solver(amrex_bottom_petsc)

       if(gr_globalMe .eq. MASTER_PE) print*, "Calling PETSc KSP solver..."
