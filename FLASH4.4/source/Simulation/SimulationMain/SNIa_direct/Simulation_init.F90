...
!  call Logfile_stampMessage(myPE, '[Simulation_init] Reading initial 1-d WD profile')
  ...
  allocate(sim_wd_temp_tab(1:sim_wd_npnts+1),STAT=istat)
    if (istat /= 0) call Driver_abortFlash("Cannot allocate space for inital WD")
  ! species read in
  allocate(sim_wd_he4_tab (1:sim_wd_npnts+1),STAT=istat)
    if (istat /= 0) call Driver_abortFlash("Cannot allocate space for inital WD")
  allocate(sim_wd_c12_tab (1:sim_wd_npnts+1),STAT=istat)
    if (istat /= 0) call Driver_abortFlash("Cannot allocate space for inital WD")
  allocate(sim_wd_o16_tab (1:sim_wd_npnts+1),STAT=istat)
    if (istat /= 0) call Driver_abortFlash("Cannot allocate space for inital WD")
  ...
  do i = 1, sim_wd_npnts
     read(2,*) sim_wd_rad_tab(i), sim_wd_dens_tab(i), sim_wd_temp_tab(i), sim_wd_he4_tab(i), sim_wd_c12_tab(i), sim_wd_o16_tab(i)
  ...
  sim_wd_dens_tab(sim_wd_npnts+1) = sim_wd_dens_tab(sim_wd_npnts)
  sim_wd_temp_tab(sim_wd_npnts+1) = sim_wd_temp_tab(sim_wd_npnts) 
  sim_wd_he4_tab(sim_wd_npnts+1) = sim_wd_he4_tab(sim_wd_npnts) 
  sim_wd_c12_tab(sim_wd_npnts+1) = sim_wd_c12_tab(sim_wd_npnts) 
  sim_wd_o16_tab(sim_wd_npnts+1) = sim_wd_o16_tab(sim_wd_npnts) 
  sim_wd_dr = dr
  ...
  sim_wd_mass_tab(sim_wd_npnts+1) = sim_wd_dens_tab(sim_wd_npnts+1) * ( sim_wd_vol_tab(sim_wd_npnts+1) - sim_wd_volume )
  !write(*,*) 'init: ',sim_wd_rad_tab(sim_wd_npnts),sim_wd_rad_tab(0),  sim_wd_vol_tab(0),  sim_wd_vol_tab(sim_wd_npnts)
