...
subroutine Driver_evolveFlash()
    ...
    real    :: domain(LOW:HIGH, MDIM) = 0.0
    integer :: domainBC(LOW:HIGH, MDIM) = PERIODIC 
    real    :: deltas(1:MDIM) = 0.0
    real    :: x_expected = 0.0
    real    :: y_expected = 0.0
    integer :: max_level = -1
    ...
    real                   :: zMin = 0.0
    real                   :: zMax = 0.0
    real                   :: boundBox(LOW:HIGH, 1:MDIM) = 0.0
    real                   :: c_lo(1:MDIM) = 0.0
    real                   :: c_hi(1:MDIM) = 0.0
    real                   :: c_gc(1:MDIM) = 0.0 
    real                   :: c_itr(1:MDIM) = 0.0
    real                   :: x_coords(4) = 0.0
    real                   :: y_coords(4) = 0.0
    real                   :: x_coords_gc(8) = 0.0
    real                   :: y_coords_gc(8) = 0.0
    real                   :: volume = 0.0
    real                   :: r = 0.0
...
!    tileDesc%level = 1
!    tileDesc%limits(LOW,  :) = [1, 1, 1]
!    call Grid_getSingleCellCoords([1, 1, 1], tileDesc, LEFT_EDGE, INTERIOR, c_lo)
!    tileDesc%limits(LOW,  :) = [57, 61, 2]
!    call Grid_getSingleCellCoords([8, 4, 1], tileDesc, RIGHT_EDGE, INTERIOR, c_hi)
!    ! Find coordinates of cell as a guard cell of one block ...
!    tileDesc%limits(LOW,  :) = [57, 61, 2]
!    call Grid_getSingleCellCoords([1, 1, 1], tileDesc, CENTER, EXTERIOR, c_gc)
!    ! and as an interior cell of its neighboring block
!    tileDesc%limits(LOW,  :) = [49, 57, 2]
!    call Grid_getSingleCellCoords([7, 3, 1], tileDesc, CENTER, INTERIOR, c_itr)
!
...
!    tileDesc%limits(LOW, :) = [1, 1, 1]
!    call Grid_getCellCoords(IAXIS, tileDesc, LEFT_EDGE, .FALSE., &
!                            x_coords, SIZE(x_coords))
!    call Grid_getCellCoords(JAXIS, tileDesc, LEFT_EDGE, .FALSE., &
...
!    tileDesc%limits(LOW, :) = [61, 61, 1]
!    call Grid_getCellCoords(IAXIS, tileDesc, RIGHT_EDGE, .FALSE., &
!                            x_coords, SIZE(x_coords))
!    call Grid_getCellCoords(JAXIS, tileDesc, RIGHT_EDGE, .FALSE., &
!                            y_coords, SIZE(y_coords))
!    do j = 1, SIZE(x_coords)
...
!
!    tileDesc%limits(LOW, :) = [61, 61, 1]
!    call Grid_getCellCoords(IAXIS, tileDesc, RIGHT_EDGE, .TRUE., &
!                            x_coords_gc, SIZE(x_coords_gc))
!    call Grid_getCellCoords(JAXIS, tileDesc, RIGHT_EDGE, .TRUE., &
...
!        tileDesc%level = 1
!        tileDesc%limits(LOW,  :) = [  1,   1, 1]
!        tileDesc%limits(HIGH, :) = [NXB, NYB, 1]
!        call Grid_getSingleCellVol(tileDesc, [1, 1, 1], volume, INTERIOR)
!        r = 0.5*dr
!        call assertEqual(volume, 2.0*PI*r*dr*dz, "Invalid cell volume")
!        call Grid_getSingleCellVol(tileDesc, [2, 1, 1], volume, INTERIOR)
!        r = 1.5*dr
!        call assertEqual(volume, 2.0*PI*r*dr*dz, "Invalid cell volume")
!        call Grid_getSingleCellVol(tileDesc, [3, 1, 1], volume, INTERIOR)
!        r = 2.5*dr
!        call assertEqual(volume, 2.0*PI*r*dr*dz, "Invalid cell volume")
!        call Grid_getSingleCellVol(tileDesc, [4, 1, 1], volume, INTERIOR)
!        r = 3.5*dr
!        call assertEqual(volume, 2.0*PI*r*dr*dz, "Invalid cell volume")
!
!        ! Moving through cells along z should not change volume
!        call Grid_getSingleCellVol(tileDesc, [1, 2, 1], volume, INTERIOR)
!        r = 0.5*dr
!        call assertEqual(volume, 2.0*PI*r*dr*dz, "Invalid cell volume")
!        call Grid_getSingleCellVol(tileDesc, [4, 2, 1], volume, INTERIOR)
!        r = 3.5*dr
!        call assertEqual(volume, 2.0*PI*r*dr*dz, "Invalid cell volume")
!    
...
!        tileDesc%limits(LOW,  :) = [  1,   1, 1]
!        tileDesc%limits(HIGH, :) = [NXB, NYB, 1]
!        call Grid_getSingleCellVol(tileDesc, [2, 1, 1], volume, EXTERIOR)
!        r = 0.5*dr
!        call assertEqual(volume, 2.0*PI*r*dr*dz, "Invalid cell volume")
!        call Grid_getSingleCellVol(tileDesc, [1, 1, 1], volume, EXTERIOR)
!        r = 1.5*dr
!        call assertEqual(volume, 2.0*PI*r*dr*dz, "Invalid cell volume")
!       
!        ! No change along z
!        call Grid_getSingleCellVol(tileDesc, [2, 2, 1], volume, EXTERIOR)
!        r = 0.5*dr
!        call assertEqual(volume, 2.0*PI*r*dr*dz, "Invalid cell volume")
