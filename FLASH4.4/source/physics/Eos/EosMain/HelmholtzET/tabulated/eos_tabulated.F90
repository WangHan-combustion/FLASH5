...
subroutine eos_tabulated(mode, vecLen, eosData, vecBegin, vecEnd, eosType, subtype, material, mask)
...
#include "Multispecies.h"
  ...
  select case (myType)
     case(EOS_TAB)
        ! ok...
     case default
        if (eos_meshMe==MASTER_PE) print*,"eos_tabulated: only supports EOS type EOS_TAB, called for eosType=", myType
        call Driver_abortFlash("eos_tabulated: only supports EOS_TAB, called for wrong EOS type!")
  end select
  ...
  select case (mySubtype)
     ...
     case default
        if (eos_meshMe==MASTER_PE) &
             print*,"eos_tabulated: only supports EOS subtype 4 and 5 and 6, called for subtype=", mySubtype
        call Driver_abortFlash("eos_tabulated: only supports EOS subtype 4 and 5 and 6, called for wrong EOS subtype!")
  end select
  ...
  if (species .LE. 0) then
     if (eos_meshMe==MASTER_PE) print*,"eos_tabulated: invalid species=",species
     call Driver_abortFlash("eos_tabulated: species number not given or invalid!")
  end if
...
#ifdef DEBUG_EOS
  print*,'Calling eos_tabulated, mode',mode,' specno',specno
#endif
...
!============================================================================
  ...
  if (ilo < 1 .OR. ilo > vecLen) then
     print*,'[eos_tabulated] ilo is',ilo
     call Driver_abortFlash("[eos_tabulated] invalid ilo")
  end if
  if (ihi < 1 .OR. ihi > vecLen) then
     print*,'[eos_tabulated] ihi is',ihi
     call Driver_abortFlash("[eos_tabulated] invalid ihi")
  end if
  if (rowLen < 0 .OR. rowLen > vecLen) then
     print*,'[eos_tabulated] rowLen is',rowLen
     call Driver_abortFlash("[eos_tabulated] invalid rowLen")
  end if
#endif
  if (rowLen == 0) then
     print*,'[eos_tabulated] rowLen is 0.'
  end if
...
!!$     useNRForAddtlCombined = .TRUE.; useNRForGamcCombined = .TRUE. !NONO in this file!
  ...
  if(maskPtr(EOS_ENTRELE)) then
     call Driver_abortFlash('eos_tabulated: computing of electron entropy is not supported!')
  end if
#endif
...
!!!!!$  end if
  ...
  do i=ilo,ihi
     call eos_gphGetTabulatedData(species=specno, &
...
!!$            eosData(tempIon+ilo:tempIon+ihi) / eosData(abar+ilo:abar+ihi)
     ...
     if(.NOT.maskPtr(EOS_PRESION))call Driver_abortFlash('eos_tabulated needs EOS_PRESION in mask!') 
...
!!$          / eosData(abar+ilo:abar+ihi)
     ...
     eosData(entr+ilo:entr+ihi) = 0.0
#ifdef DEBUG_EOS
     print*,'presIon by tabIonmix4:', eosData(presIon+ilo:presIon+ihi)
#endif
...
!!$          eosData(tempEle+ilo:tempEle+ihi) / eosData(abar+ilo:abar+ihi) * dynamicZ
     ...
     if(.NOT.maskPtr(EOS_PRESELE))call Driver_abortFlash('eos_tabulated needs EOS_PRESELE in mask!') 
...
#ifdef DEBUG_EOS
     print*,'presEle by tabIonmix4:', eosData(presEle+ilo:presEle+ihi)
#endif
...
9996 format(' E.o.MODE_DENS_TEMP_EQUI det',1P,4(G20.6,1x))
  ...
  case default
     if (eos_meshMe==MASTER_PE) then
        print*,"eos_tabulated: invalid mode=",mode
     ...
     call Driver_abortFlash("[eos_tabulated] Unrecognized input mode given to eos_tabulated")
  end select
...
#ifdef DEBUG_EOS
  print*,'gamc a.e.o.tabIonmix4:', eosData(gamc+ilo:gamc+ihi)
#endif
...
!!$           eos_gammaRad*eosData(presRad+ilo:presRad+ihi) ) / eosData(pres+ilo:pres+ihi)
    ...
    stop 'called setCombinedGamc in eos_tabulated'
!!$#else
