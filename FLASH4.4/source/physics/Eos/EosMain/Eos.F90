...
#include "Flash.h"
  ...
  if(pMassFrac_and_mask) then
     ...
     case(EOS_TAB)
        call eos_tabulated(mode, vecLen, eosData, vecBegin,vecEnd, mask=mask, eosType=eos_type)
     case(EOS_NUC)
  ...
  elseif (pMassFrac) then
     ...
     case(EOS_TAB)
        call eos_tabulated(mode, vecLen, eosData, vecBegin,vecEnd, eosType=eos_type)
     case(EOS_NUC)
